    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            for obj_idx in range(batch_size):
                ...
                # Remove old frame outputs to save memory
                storage_key, obj_key = "non_cond_frame_outputs", "output_dict_per_obj"
                
                if reverse:
                    # When tracking backwards, remove frames that are too far ahead
                    newest_allowed_idx = frame_idx + 16
                    all_frame_idxs = obj_output_dict[storage_key].keys()
                    old_frame_idxs = [idx for idx in all_frame_idxs if idx > newest_allowed_idx]
                else:
                    # When tracking forwards, remove frames that are too far behind
                ...
                for old_idx in old_frame_idxs:
                    ...
                    for objid in inference_state[obj_key].keys():
                        if old_idx in inference_state[obj_key][objid][storage_key]:
